#!/usr/bin/env python3
"""
Task 96 Core Verification Script (Simple Version)
Focused testing of core cost tracking functionality without emojis
"""
...
def verify_core_functionality():
    ...
    try:
        ...
        if migrations_path.exists():
            ...
            print(f"[PASS] Migrations: {len(migration_files)} migration files found")
        else:
            results['tests']['migrations'] = {
                'status': 'FAIL',
            ...
            print("[FAIL] Migrations: directory not found")
    except Exception as e:
        results['tests']['migrations'] = {'status': 'FAIL', 'error': str(e)}
        print(f"[FAIL] Migrations: {e}")
    ...
    try:
        ...
        if missing_modules:
            ...
            print(f"[FAIL] Core Modules: Missing {missing_modules}")
        else:
            results['tests']['core_modules'] = {
                'status': 'PASS',
            ...
            print(f"[PASS] Core Modules: {len(existing_modules)} modules found")
    except Exception as e:
    ...
    try:
        ...
        if env_example_path.exists():
            ...
            if missing_vars:
                ...
                print(f"[PARTIAL] Configuration: Missing vars {missing_vars}")
            else:
                print(f"[PASS] Configuration: All {len(found_vars)} cost tracking vars documented")
        else:
            results['tests']['configuration'] = {
                'status': 'FAIL',
            ...
            print("[FAIL] Configuration: .env.example not found")
    except Exception as e:
    ...
    try:
        ...
        if migrations_path.exists():
            ...
            for migration_file in migration_files:
                try:
                    ...
                    print(f"[FAIL] Migration {migration_file.name} failed: {e}")
                    break
            ...
            if missing_tables:
                ...
                print(f"[FAIL] Database Migration: Missing tables {missing_tables}")
            else:
                results['tests']['database_migration'] = {
                    'status': 'PASS',
                ...
                print(f"[PASS] Database Migration: {len(tables_created)} tables created")
        ...
        print(f"[FAIL] Database Migration: {e}")
    ...
    try:
        ...
        if calculation_correct:
            ...
            print(f"[PASS] Cost Calculation: ${total_cost:.4f}")
        else:
            results['tests']['cost_calculation'] = {
                'status': 'FAIL',
                'calculated_cost': total_cost,
            ...
            print(f"[FAIL] Cost Calculation: Expected ${expected_total:.4f}, got ${total_cost:.4f}")
    except Exception as e:
    ...
    try:
        ...
        if chat_session_path.exists():
            ...
            if integration_status == 'PASS':
                ...
                print(f"[FAIL] Integration Points: {', '.join(issues)}")
        else:
            results['tests']['integration_points'] = {
                'status': 'FAIL',
            ...
            print("[FAIL] Integration Points: chat_session.py not found")
    except Exception as e:
        results['tests']['integration_points'] = {'status': 'FAIL', 'error': str(e)}
        print(f"[FAIL] Integration Points: {e}")
    ...
    try:
        ...
        if missing_ui:
            ...
            print(f"[PARTIAL] UI Components: Missing {missing_ui}")
        else:
            results['tests']['ui_components'] = {
                'status': 'PASS',
            ...
            print(f"[PASS] UI Components: {len(existing_ui)} components found")
    except Exception as e:
        results['tests']['ui_components'] = {'status': 'FAIL', 'error': str(e)}
        print(f"[FAIL] UI Components: {e}")
    ...
    if results['overall_status'] == 'PASS':
        ...
        results['recommendations'].append("Task 96 is functional but has minor issues to address")
